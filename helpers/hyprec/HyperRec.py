# Methods to compute the reciprocity of a directed hypergraph
import io
...
from helpers.config import alpha, met_path
import sys
sys.path.insert(1, '../')
...
def efficient_distance_matching(true_S, true_T,
                                e, candidates,
    ...
    if len(candidates) == 1:  # 1 vs 1 simple matching
        ...
        if is_sparse:  # Sparse matrix
            true_probs = np.array(true_T[:, e].todense()).reshape(
                1, -1)[0] / np.sum(true_T[:, e])
            candid = candidates[0]
            our_dist = np.array(true_S[:, candid].todense()).reshape(
                1, -1)[0] / np.sum(true_S[:, candid])
        ...
        if is_sparse:
            true_probs = np.array(true_T[:, e].todense()).reshape(
                1, -1)[0] / np.sum(true_T[:, e])
            partialS = set(np.where(true_S[:, candidates].todense() > 0)[
                           0])  # Target Arc Nodes are inside here
            # Candidate Nodes are inside here
        ...
        for i, head_ind in enumerate(head_index[e]):
            for j, tail_ind in enumerate(candidates):
                if head_ind in tail_index[tail_ind]:
                    # Intersection among head & tail set nodes
                    head_tail_intersect[i, j] = 1
        ...
        if special_case == "FDHG":  # Iteratively add one by one
            ...
            for cardin_ in range(1, (len(candidates) + 1)):
                ...
                if cardin_ == 1:
                    ...
                    # |S| * |C'|
                    new_S = head_tail_intersect[:, candid_e].reshape(-1)
                    new_P = full_probs[candid_e, :]  # C' x V'

                    new_P = new_P.reshape(-1)
                    try:
                        penalty = (distance.jensenshannon(
                            true_probs, new_P)) ** 2
                    except:
                        penalty = MAX
                    ...
                    for tmp_e in range(new_P.shape[0]):
                        ...
                        if np.sum(new_P[tmp_e, :]) == 0:
                            ...
                            try:
                                penalty += (distance.jensenshannon(true_probs,
                                            new_P[tmp_e, :])) ** 2
                            except:
                                penalty += MAX

                    cur_recip = (
                        1 - (penalty) / (new_P.shape[0] * MAX))*(((1 / len(candid_e)) ** alpha))
            ...
            for cardin_ in range(1, (len(candidates) + 1)):
                ...
                cur_candid = list(itertools.combinations(
                    entire_candids, cardin_))
                ...
                if cardin_ == 1:
                    for combin in cur_candid:
                        ...
                        cur_recip = 1 - (penalty * np.sum(new_S) + MAX *
                                         (new_S.shape[0] - np.sum(new_S))) / (MAX * new_S.shape[0])
...
def measure_reciprocity(head_index, tail_index,
                        data_type='normal_sparse',
                        omega_dist={},
                        given_interval=np.arange(10),
                        alpha=1e-6,
                        special_case="normal"):
    ...
    if data_type == 'normal_sparse':
        ...
        for e in range(omega_dist.shape[0]):

            is_pass = False

            # non-reciprocal hyperedge: reciprocity is 0
            if omega_dist[e] == 0:
                continue
            else:
                ...
                H_ = np.zeros(
                    (candids.shape[0], (len(true_headset)) + len(true_tailset)))
                # Head and tail size
                size_order = np.zeros((candids.shape[0], 2))
                ...
                # search space \Psi with reduced number of candidates
                candids_list = []
                for cur_edge in np.unique(tmp_entire):
                    # for each group of edges with the same overlaps in the heads and the tails
                    # we take the hyperedge with the min head size
                    inds = np.where(tmp_entire == cur_edge)[0]
                ...
                for c_e in candids_list:
                    ...
                    # perfectly reciprocal hyperedge: reciprocity is 1
        ...
        for e in tqdm(given_interval):
            ...
            if omega_dist[e] == []:  # No Overlapping
                ...
                H_ = np.zeros(
                    (len(candids), (len(true_headset)) + len(true_tailset)))
                ...
                for cur_edge in np.unique(tmp_entire):
                    # They are group of edges which overlap on the same place
                    inds = np.where(tmp_entire == cur_edge)[0]
                    head_min = np.argmin(size_order[inds, 0])
                    candids_list.append(candids[inds[head_min]])
...
def bitcoin_dataset_reciprocity_computation(head_index, tail_index, alpha):
    ...
    for i in (range(interval.shape[0] - 1)):
        ...
        print(
            "Preprocessing {0}/{1} is on process".format(i + 1, interval.shape[0] - 1))
...
def parallel_reciprocity(inp):
    ...
    with z.open(file_name, 'r') as f1:
        ...
        heads_t, tails_t = read_as_directed_hyperedge_list(fobj, vmap)
    heads_t = np.array([set(x) for x in heads_t])
    tails_t = np.array([set(x) for x in tails_t])

    recip_vect = measure_reciprocity(heads_t,
                                     tails_t,
                                     alpha=alpha,
                                     special_case="normal")
    with open(f'{met_path}recip_{map_fields["algorithm"]}_{map_fields["randomSeed"]}.tsv', 'w') as o_f:
        for idx, r in enumerate(recip_vect):
            o_f.write(f'{idx}\t{r}\n')
    print(
        f'Found reciprocity for Sample {map_fields["randomSeed"]} of Sampler {map_fields["algorithm"]}')
    # Sampler, Sample, Reciprocity, HyperEdge Id
    output = []
    for idx, r in enumerate(recip_vect):
        output.append([map_fields['algorithm'], str(
            map_fields['randomSeed']), str(r), str(idx)])
    return output
